...
class clothing_dataset(Dataset): 
    def __init__(self, root, transform, mode, num_samples=0, pred=[], probability=[], paths=[], num_class=14, idx_remove=[]): 
        ...
        if mode == 'all':
            ...
            pred_idx = [i for i in temp_indices if i not in idx_remove]     
            self.train_imgs = [train_imgs[i] for i in pred_idx]                
            self.probability = [probability[i] for i in pred_idx]            
            print("%s data has a size of %d"%(self.mode,len(self.train_imgs)))
                    
    def __getitem__(self, index):  
        if self.mode=='labeled' or self.mode=='proportional':
...
class clothing_dataloader():  
    ...
    def run(self,mode,pred=[],prob=[],paths=[], idx_remove=[]):        
        if mode=='warmup':
            ...
            train_dataset = clothing_dataset(self.root,transform=self.transform_train, mode='proportional',pred=pred, probability=prob,paths=paths,idx_remove=idx_remove)
