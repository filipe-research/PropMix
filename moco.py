...
def main():
    ...
    memory_bank_train = MemoryBank(len(train_dataset), backbone_dim, p['num_classes'], p['temperature'])
    memory_bank_train.cuda()
    memory_bank_val = MemoryBank(len(val_dataset), backbone_dim, p['num_classes'], p['temperature'])
    ...
    if args.dataset == 'imagenet':
        ...
        moco_state = torch.load('moco_pretrained/webvision.tar', map_location='cpu')
    ...
    if args.dataset == 'imagenet':
        ...
        for k in list(state_dict.keys()):
            ...
            if k.startswith('module.fc'):
                new_k = 'module.contrastive_head.' + k[len('module.fc.'):]
                new_state_dict[new_k] = state_dict[k]
            
            else:
                new_k = 'module.backbone.' + k[len('module.'):]
                new_state_dict[new_k] = state_dict[k]
